<script>
    ...
    let state = {oldClassicalBoxWithCat: undefined, scene: undefined, classicalState: false, particlesArr: new Array(10).fill(undefined), particlesDirectionArr: new Array(10).fill(undefined)};

    let demoState = 0;

    let demoTextArr = ["Begin Experiment", "Open the box", "Finished"];

    async function reset() {
        state = state = {oldClassicalBoxWithCat: undefined, scene: state.scene, classicalState: false, particlesArr: new Array(10).fill(undefined), particlesDirectionArr: new Array(10).fill(undefined)};
        ...
        initScene();
        addBoxWithCat(1, false, 2);
        state.oldClassicalBoxWithCat = await addBoxWithCat(2, false, 2);
    }
    ...
    function animateDecoherenceParticles(step=0.05, ballSize=0.3, numParticles = 5) {
        if (state.particlesArr.reduce((prevValue, curValue) => {
            ...
            addDecoherenceParticle(getRandomNumberInRange([[0.5, 8], [28, 36]]), getRandomNumberInRange([[-18, -8], [8, 18]]), getRandomNumberInRange([[-18, -8], [8, 18]]), ballSize);
        }
        for (let i in state.particlesArr.reduce((prevValue, curValue, curIndex) => {
            ...
            if ((state.particlesArr[i].sphere.position.x >= 9-ballSize/2 && state.particlesArr[i].sphere.position.x <= 27+ballSize/2) &&
                (state.particlesArr[i].sphere.position.y >= -7-ballSize/2 && state.particlesArr[i].sphere.position.y <= 7+ballSize/2) &&
                (state.particlesArr[i].sphere.position.z >= -7-ballSize/2 && state.particlesArr[i].sphere.position.z <= 7+ballSize/2)
                ) {
                ...
                if (state.oldClassicalBoxWithCat != undefined) {
                    ...
                    addBoxWithCat(2, false, (state.classicalState) ? 1 : 0);
                    state.oldClassicalBoxWithCat = undefined;
                }
                addLabelToParticle(i);
            }
            const xDiff = 18 - state.particlesArr[i].sphere.position.x;
            const yDiff = 0 - state.particlesArr[i].sphere.position.y;
            const zDiff = 0 - state.particlesArr[i].sphere.position.z;
            ...
            if ((state.particlesArr[i].sphere.position.x < ballSize/2 || state.particlesArr[i].sphere.position.x > 36-ballSize/2) ||
                (state.particlesArr[i].sphere.position.y < -18+ballSize/2 || state.particlesArr[i].sphere.position.y > 18-ballSize/2) ||
                (state.particlesArr[i].sphere.position.z < -18+ballSize/2 || state.particlesArr[i].sphere.position.z > 18-ballSize/2)
                ) {
    ...
    async function nextDemoStep() {
        if (demoState == 0) {
            ...
            initScene();
            if (Math.random() > 0.5) {
                state.classicalState = false;
            }
            else {
                state.classicalState = true;
            }
            addBoxWithCat(1, false, 2);
            state.oldClassicalBoxWithCat = await addBoxWithCat(2, false, 2);
        }
        if (demoState == 1) {
            ...
            initScene();
            if (!state.classicalState) {
                addBoxWithCat(0, true, 1);
                addBoxWithCat(1, true, 0);
                addBoxWithCat(2, true, 0);
            }
            else {
                addBoxWithCat(0, true, 0);
                addBoxWithCat(1, true, 1);
                addBoxWithCat(2, true, 1);
            }
            addEqualSign();
            createSquareFrame();
        }
    ...
    function initScene() {
        ...
        let labelClassical = new TextSprite({
            text: 'Classical (With Decoherence)',
            fontSize: 2,
            color: '#000000',
        });
    ...
    function addEqualSign() {
        const lineGeometry0 = new THREE.CylinderGeometry(0.2, 0.2, 6, 3, 1);
        const lineMaterial0 = new THREE.MeshStandardMaterial({color: 0x00ff00});
        const line0 = new THREE.Mesh(lineGeometry0, lineMaterial0);
        line0.translateY(0.5);
        line0.rotateZ(Math.PI/2);
        state.scene.add(line0);

        const lineGeometry1 = new THREE.CylinderGeometry(0.2, 0.2, 6, 3, 1);
        const lineMaterial1 = new THREE.MeshStandardMaterial({color: 0x00ff00});
        const line1 = new THREE.Mesh(lineGeometry1, lineMaterial1);
        line1.translateY(-0.5);
        line1.rotateZ(Math.PI/2);
        state.scene.add(line1);
    }

    function addCorrelationSign(x) {
        const scene = state.scene;

        const lineGeometry = new THREE.CylinderGeometry(0.5, 0.5, 3, 3, 1);
        ...
        const arrowGeometryT = new THREE.ConeGeometry(1, 1.5, 10, 1);
        const arrowMaterialT = new THREE.MeshStandardMaterial({color: 0x00ffff});
        const arrowT = new THREE.Mesh(arrowGeometryT, arrowMaterialT);
        arrowT.translateY(-1.5+10);
        arrowT.translateX(x);
        arrowT.rotateZ(Math.PI);
        scene.add(arrowT);

        const arrowGeometryB = new THREE.ConeGeometry(1, 1.5, 10, 1);
        const arrowMaterialB = new THREE.MeshStandardMaterial({color: 0x00ffff});
        const arrowB = new THREE.Mesh(arrowGeometryB, arrowMaterialB);
        arrowB.translateY(1.5+10);
        arrowB.translateX(x);
        scene.add(arrowB);
    }
    ...
    function createSquareFrame() {
        ...
        let label = new TextSprite({
            text: '|Universe⟩|Cat Dead⟩ + |Universe⟩|Cat Alive⟩\n____________________________________\n√2',
            fontSize: 1,
            color: '#000000',
        });
        label.translateX(-29);
        ...
        addCorrelationSign(-18);
        addCorrelationSign(-40);
    }
    ...
    afterUpdate(async () => {
        ...
        document.getElementById("schrodinger-container").appendChild(renderer.domElement);
...
<div id="schrodinger-container">
    <div class="controls-container">
        <button id="demo-button" style="background-color:rgba(160, 255, 160);" on:click={nextDemoStep} disabled={demoState > 1}>{demoTextArr[demoState]}</button>
